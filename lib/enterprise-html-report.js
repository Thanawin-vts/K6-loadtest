...
const METRIC_DISPLAY_LABELS = {
  ...
  bidding_success_duration: 'เวลาส่งบิดจนได้ bidInfo ( bidding_success_duration )',
  bidding_failure_duration: 'เวลาส่งบิดจนได้ notification level=error ( bidding_failure_duration )',
...
function trendStats(data, name) {
  const values = (data && data.metrics && data.metrics[name] && data.metrics[name].values) || {};
  return {
...
function buildFlowSummaryFromMetrics(data) {
  ...
  return {
    ...
    bidOutcome: {
      success: trendStats(data, 'bidding_success_duration'),
      failure: trendStats(data, 'bidding_failure_duration'),
    },
    reconnect: {
      zone: 'postrequisite',
...
export function parseK6Metrics(data, options) {
  ...
  const bidSuccessTrend = trendStats(data, 'bidding_success_duration');
  const bidFailureTrend = trendStats(data, 'bidding_failure_duration');
  ...
  const perfZoneActive = perfSamples > 0 || bidSuccessTrend.count > 0 || bidFailureTrend.count > 0;
  ...
  return {
    meta: {
      ...
      perfP95Ms: perfP95Ms,
      perfP99Ms: perfP99Ms,
      biddingSuccessDuration: bidSuccessTrend,
      biddingFailureDuration: bidFailureTrend,
...
export function generateEnterpriseHtmlReport(data, options) {
  ...
  const bidOutcome = flow.bidOutcome || {};
  const bidSuccess = bidOutcome.success || {};
  const bidFailure = bidOutcome.failure || {};
  const durationCard = (label, stats) => {
    const count = Number(stats.count) || 0;
    if (!count) {
      return `<div class="flow-stat">
        <div class="flow-stat-label">${label}</div>
        <strong class="flow-stat-value mono">not recorded</strong>
        <div class="flow-stat-sub">ยังไม่มีตัวอย่างที่จับคู่ได้</div>
      </div>`;
    }
    const n = value => Math.round(Number(value) || 0);
    return `<div class="flow-stat">
      <div class="flow-stat-label">${label}</div>
      <strong class="flow-stat-value mono">${n(stats.avg).toFixed(1)} <span style="font-size: 0.9rem; font-weight: 400;">avg ms</span> · ${n(stats.p95).toFixed(1)} <span style="font-size: 0.9rem; font-weight: 400;">p95 ms</span></strong>
      <div class="flow-stat-sub">count ${count.toLocaleString()} · min ${n(stats.min).toFixed(1)} · med ${n(stats.med).toFixed(1)} · p90 ${n(stats.p90).toFixed(1)} · max ${n(stats.max).toFixed(1)} ms</div>
    </div>`;
  };
...
<div class="dashboard-container">
  ...
  <section class="kpi-grid">
    ...
    <div class="kpi-card pass">
      <div class="kpi-label">ส่งบิด → bidInfo ( bidding_success_duration ) <i class="fa-solid fa-circle-check"></i></div>
      <div class="kpi-value mono">${(meta.biddingSuccessDuration && meta.biddingSuccessDuration.count > 0) ? meta.biddingSuccessDuration.avg.toFixed(1) : '—'} <span style="font-size: 1rem; font-weight: 400;">ms</span></div>
      <div class="kpi-sub">${(meta.biddingSuccessDuration && meta.biddingSuccessDuration.count > 0)
        ? 'p95: ' + meta.biddingSuccessDuration.p95.toFixed(1) + ' ms · count ' + meta.biddingSuccessDuration.count.toLocaleString()
        : 'ยังไม่มีตัวอย่างที่จับคู่ได้'}</div>
    </div>

    <div class="kpi-card ${(meta.biddingFailureDuration && meta.biddingFailureDuration.count > 0) ? 'warning' : 'pass'}">
      <div class="kpi-label">ส่งบิด → error ( bidding_failure_duration ) <i class="fa-solid fa-circle-xmark"></i></div>
      <div class="kpi-value mono">${(meta.biddingFailureDuration && meta.biddingFailureDuration.count > 0) ? meta.biddingFailureDuration.avg.toFixed(1) : '—'} <span style="font-size: 1rem; font-weight: 400;">ms</span></div>
      <div class="kpi-sub">${(meta.biddingFailureDuration && meta.biddingFailureDuration.count > 0)
        ? 'p95: ' + meta.biddingFailureDuration.p95.toFixed(1) + ' ms · count ' + meta.biddingFailureDuration.count.toLocaleString()
        : 'ยังไม่มีตัวอย่างที่จับคู่ได้'}</div>
    </div>
  ...
  <section class="card flow-summary">
    ...
    <div class="flow-summary-grid">
      ...
      <div class="flow-stat">
        ...
        <strong class="flow-stat-value mono">${(Number(acknowledgement.waitAvgMs) || 0).toFixed(1)} <span style="font-size: 0.9rem; font-weight: 400;">avg ms</span> · ${Math.round(Number(acknowledgement.waitP95Ms) || 0)} <span style="font-size: 0.9rem; font-weight: 400;">p95 ms</span></strong>
      ...
      ${durationCard('เวลาส่งบิดจนได้ bidInfo ( bidding_success_duration )', bidSuccess)}
      ${durationCard('เวลาส่งบิดจนได้ notification level=error ( bidding_failure_duration )', bidFailure)}
    </div>
