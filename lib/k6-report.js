...
function trendLatency(data, name) {
...
function collectTrendSummaries(data) {
  ...
  const preferredNames = [
    ...
    'ws_ack_wait_ms',
    'bidding_success_duration',
    'bidding_failure_duration',
  ];
  ...
  return {
    ...
    phases: {
      ...
      bidding: {
        ...
        successDuration: trendLatency(data, 'bidding_success_duration'),
        failureDuration: trendLatency(data, 'bidding_failure_duration'),
      },
      postrequisite: {
    ...
    flow: {
      ...
      bidOutcome: {
        success: trendLatency(data, 'bidding_success_duration'),
        failure: trendLatency(data, 'bidding_failure_duration'),
      },
...
export function formatReadableReportSummary(summary) {
  ...
  const lat = b.latency || {};
  const successLat = b.successDuration || {};
  const failureLat = b.failureDuration || {};
  function durationLines(title, stats) {
    const d = stats || {};
  ...
  const lines = [
    ...
    '',
  ].concat(durationLines('เวลาส่งบิดจนได้ bidInfo ( bidding_success_duration ):', successLat)).concat([
    '',
  ]).concat(durationLines('เวลาส่งบิดจนได้ notification level=error ( bidding_failure_duration ):', failureLat)).concat([
    '',
    'อัตรา ( Rates ):',
