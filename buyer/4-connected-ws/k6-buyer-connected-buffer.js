/**
 * k6 — login → [setup: lot-bidder-number ทีละ user] → WS visitLot → WS connected
 *
 ...
 *        phase หลัก: login + WS visitLot → connected ยังรันพร้อมกันตาม VU
 *
 * รันตัวอย่าง (parallel + hold connection):
 *   ./buyer-connected-buffer-script.sh 975 5m 1 10 loadtestuser
 *   k6 run buyer/4-connected-ws/k6-buyer-connected-buffer.js \
 ...
 *   REPORT_BASENAME=buyer-connected-buffer
...
function runWebsocketVisitConnected(session, bidderNumber) {
  ...
  if (failed) {
    return {
      ok: false,
      error: { step: 'ws_connected', reason: 'unknown' },
    };
  }
...
export default function (setupData) {
  ...
  let wsResult;
  group(`3. WS visitLot → connected (${buyer.username})`, () => {
  ...
  if (wsResult && !wsResult.ok) {
    ...
    logError(
      'iteration.abort',
      `step=${err.step || 'ws_connected'} reason=${err.reason || 'unknown'} user=${buyer.username} lotId=${LOT_ID} code=${err.code || '-'} message=${err.message || err.detail || '-'} httpStatus=${err.httpStatus || '-'}`
    );
    sleep(1);
    return;
  }
...
export const handleSummary = createHandleSummary(function () {
  return {
    titleBase: __ENV.REPORT_TITLE || 'k6 login → lot-bidder (setup) → visitLot → connected',
    reportDir: __ENV.REPORT_DIR || 'k6-reports',
    reportBasename: __ENV.REPORT_BASENAME || 'buyer-connected-buffer',
    meta: {
      baseUrl: BASE_URL,
      wsUrl: WS_URL,
      lotId: LOT_ID,
