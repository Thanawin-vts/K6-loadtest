/**
 * k6 — login → lot-bidder-number → WS visitLot
 *
 * BUYER_USER = array ของ buyer
 * Default: เปิด VU พร้อมกัน = จำนวน buyer (1 VU : 1 user) แล้วแต่ละ VU รันครบทุก action
 *
 * บริบท: ทุก buyer connect แล้วส่ง visitLot แล้วค้าง WebSocket ไว้ในระบบ (ไม่ส่ง connected)
 *
 * รันตัวอย่าง (parallel + hold connection):
 *   ./buyer-visit-lot-script.sh 975 5m 1 10 loadtestuser
 *   k6 run buyer/3-visitlot-ws/k6-buyer-visit-lot.js \
 ...
 *   REPORT_BASENAME=buyer-visit-lot
    ...
    function (socket) {
      ...
      socket.on('message', function (raw) {
        ...
        if (!joinDone && visitSent && isJoinError(msg)) {
          ...
          console.error(
            `[ERROR][vu=${__VU} iter=${__ITER}] ws.visitLot.fail user=${buyer.username} lotId=${LOT_ID} msg=${raw}`
          );
          socket.close();
...
export default function () {
  ...
  group(`3. WS visitLot (${buyer.username})`, () => {
    runWebsocketVisitLot(session, bidderNumber);
  });
...
export const handleSummary = createHandleSummary(function () {
  return {
    titleBase: __ENV.REPORT_TITLE || 'k6 login → lot-bidder → visitLot',
    reportDir: __ENV.REPORT_DIR || 'k6-reports',
    reportBasename: __ENV.REPORT_BASENAME || 'buyer-visit-lot',
    meta: {
      baseUrl: BASE_URL,
      wsUrl: WS_URL,
      lotId: LOT_ID,
