...
#
# usage:
#   ./buyer-visit-lot-buffer-script.sh [lotId] [wsHold] [startLoopIndex] [endLoopIndex] [usernamePrefix] [vus]
#
# example:
...
#
# Depends on:
#   ./k6-buyer-visit-lot-buffer.js  # setup: lot-bidder ทีละ user → parallel WS visitLot
#   ../../buyer-mock-user.js
#   ../../lib/k6-report.js
#
...
K6_SCRIPT="${SCRIPT_DIR}/k6-buyer-visit-lot-buffer.js"
...
K6_ARGS=(
  ...
  -e "REPORT_BASENAME=buyer-visit-lot-buffer"
  -e "REPORT_TITLE=buyer visitLot (buffer)"
)
...
K6_WEB_DASHBOARD=true K6_WEB_DASHBOARD_EXPORT=${REPORT_DIR}/buyer-visit-lot-buffer-dashboard.html k6 run "$K6_SCRIPT" "${K6_ARGS[@]}"
...
Scenario        : buyer visitLot (buffer — lot-bidder setup sequential)
Lot ID          : $LOT_ID
WS Hold         : $WS_HOLD
...
echo "Reports: ${REPORT_DIR}/buyer-visit-lot-buffer.{json,html}"
