...
#
# usage:
#   ./buyer-send-offer-buffer-script.sh [lotId] [lotLineId] [auctionNo] [wsHold] [startLoopIndex] [endLoopIndex] [usernamePrefix] [vus]
#
# example:
#   ./buyer-send-offer-buffer-script.sh 975 10360 1 5m 1 10 loadtestuser 10
#
# Optional env (passed through to k6):
#   OFFER_EVENT, OFFER, ACK, BID_AFTER_OFFER, STAGGER_MS,
...
#
# Depends on:
#   ./k6-buyer-send-offer-buffer.js  # setup: lot-bidder ทีละ user → parallel WS offer
#   ../../buyer-mock-user.js
#   ../../lib/k6-report.js
#
...
if [ "$#" -gt 8 ]; then
  echo "usage: $0 [lotId] [lotLineId] [auctionNo] [wsHold] [startLoopIndex] [endLoopIndex] [usernamePrefix] [vus]"
...
K6_SCRIPT="${SCRIPT_DIR}/k6-buyer-send-offer-buffer.js"
...
K6_ARGS=(
  ...
  -e "REPORT_BASENAME=buyer-send-offer-buffer"
  -e "REPORT_TITLE=buyer send offer (buffer)"
)
...
K6_WEB_DASHBOARD=true K6_WEB_DASHBOARD_EXPORT=${REPORT_DIR}/buyer-send-offer-buffer-dashboard.html k6 run "$K6_SCRIPT" "${K6_ARGS[@]}"
...
Scenario        : buyer send offer (buffer — lot-bidder setup sequential)
Lot ID          : $LOT_ID
...
echo "Reports: ${REPORT_DIR}/buyer-send-offer-buffer.{json,html}"
